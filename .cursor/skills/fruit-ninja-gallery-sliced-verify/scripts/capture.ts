 ...
 *
 * From repo root:
 *   npx tsx .claude/skills/fruit-ninja-gallery-sliced-verify/scripts/capture.ts \
 *     --fruit apple --out /tmp/gallery-sliced-apple.png
 *
...
function parseArgs(): { fruit: string; out: string; baseUrl: string; headed: boolean; spin: number } {
  ...
  if (!fruit || !out) {
    console.error(
      'Usage: npx tsx .claude/skills/fruit-ninja-gallery-sliced-verify/scripts/capture.ts --fruit <kind> --out <path> [--spin N] [--base-url <url>] [--headed]',
    )
    process.exit(1)
  }
...
async function main() {
  ...
  const url = new URL(
    `/games/fruit-ninja/gallery-sliced?fruit=${encodeURIComponent(fruit)}${staticMode ? '&static=1' : ''}`,
    `${baseUrl}/`,
  )
  ...
  try {
    ...
    await page.waitForSelector('[data-testid="fruit-gallery-sliced-canvas"]', { state: 'attached', timeout: 60_000 })
    await page.waitForFunction(
      () => {
        const el = document.querySelector('[data-testid="fruit-gallery-sliced-canvas"]') as HTMLCanvasElement | null
    ...
    if (staticMode) {
      ...
      const buf = await page.locator('[data-testid="fruit-gallery-sliced-canvas"]').screenshot({ type: 'png' })
