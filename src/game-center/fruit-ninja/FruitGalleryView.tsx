...
export default function FruitGalleryView() {
  ...
  useEffect(() => {
    ...
    // The game's camera is centered at y=0.55. Align the grid to that so gallery framing matches in-game.
    const centerY = 0.55

    // Place fruits (same mesh/material factory as the game)
    for (let i = 0; i < FRUIT_KINDS.length; i++) {
      const { kind } = FRUIT_KINDS[i]!
      const radius = FRUIT_RADIUS[kind]
      // `skin` tint is still supplied for fruits that use it (some archetypes override with cached mats).
      const skinByKind: Record<FruitArchetype, number> = {
        watermelon: 0x287a38,
        apple: 0xcc2228,
        banana: 0xf0c830,
        lemon: 0xf5e050,
      ...
      const mesh = createFruitMesh(radius, kind, skinByKind[kind])
      const col = i % COLS
    ...
    // Add text sprites below each item
    ...
    // Frame the whole grid in view for any aspect ratio using measured bounds.
    const scratchBox = new THREE.Box3()
    const scratchSize = new THREE.Vector3()
    const scratchCenter = new THREE.Vector3()
    const fovRad = (camera.fov * Math.PI) / 180
    const fitCamera = (aspect: number) => {
      // Compute bounds of just the grid contents (not the dojo wall).
      ...
      // Required distance so that vertical + horizontal extents fit.
    ...
    // Higher-contrast grid for readability against dark scene background
