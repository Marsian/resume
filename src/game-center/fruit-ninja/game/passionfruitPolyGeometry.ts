...
function buildPassionfruitCustomGeometry(
  ...
  return buildPassionfruitGeometryFromThetaSteps(radius, lonSegments, buildThetaSteps(thetaStart, thetaLength))
}

function buildPassionfruitGeometryFromThetaSteps(
  radius: number,
  lonSegments: number,
  thetaSteps: number[],
  mirrorY = false,
...
export function getPassionfruitHalfPolyGeometry(radius: number): THREE.BufferGeometry {
  return getPassionfruitSlicedHalfPolyGeometry(radius, 'top')
}

export function getPassionfruitSlicedHalfPolyGeometry(radius: number, half: 'top' | 'bottom'): THREE.BufferGeometry {
  ...
  if (!g) {
    if (half === 'top') {
      ...
      g = buildPassionfruitGeometryFromThetaSteps(radius, LON_SEGMENTS, bottomThetaSteps, true)
    }
    halfCache.set(key, g)
  }
  return g
